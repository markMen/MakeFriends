	...
	<view class="container">
		<view class="tui-order-header">
			...
			<view class="tui-header-content">
				<view>
					<view class="tui-status-text">购买的订单 {{app.getStatusText(order.state)}}</view>
		...
		<view class="tui-order-item">
			...
			<view class="rows pl pr  ">
                ...
                <view class="form-text-right" v-if="order.state == '200'">
			...
			<view class="box-wrap-start pl pr">
                <view class='col img-box pr pt' v-for="(item,index) in listPhoto" :key="index">
                    ...
                    <button class='btn-danger  full-width text-center-hv' :data-index='index' @tap='onDelImg' v-if="order.state == '200'">
            ...
            <view class="box-wrap-start pl pr">
                <view class='col img-box pr pt' v-for="(item,index) in listPhotoLocal" :key="index">
                    ...
                    <button class='btn-danger  full-width text-center-hv' :data-index='index' @tap='onDelImgLocal' v-if="order.state == '200'">
                        <!-- <image src="/static/logo.png" class="img-del"></image> -->
                        x
                    </button>
                </view>
		...
		<view class="tui-tabbar tui-order-btn">
			...
			<view class="tui-btn-mr pr" v-if="order.state == '2'" @tap="onReOrder">
				<tui-button type="black" :plain="true" width="148rpx" height="56rpx" :size="26" shape="circle">申请退款</tui-button>
			</view>

			<view class="tui-btn-mr pr" v-if="order.state == '3'" @tap="onReOrderProduct">
				<tui-button type="black" :plain="true" width="148rpx" height="56rpx" :size="26" shape="circle">申请退货</tui-button>
			</view>
			
			
<!-- 			<view class="tui-btn-mr pr" v-if="order.state == '5' || order.state == '51'" @tap="onReOrderCancel">
				<tui-button type="black" :plain="true" width="148rpx" height="56rpx" :size="26" shape="circle">取消退款</tui-button>
			</view>
			<view class="tui-btn-mr pr" v-if="order.state == '52' || order.state == '53'" @tap="onReOrderProductCancel">
				<tui-button type="black" :plain="true" width="148rpx" height="56rpx" :size="26" shape="circle">取消退货</tui-button>
			</view> -->


			<view class="tui-btn-mr pr" @tap="onOkOrder" v-if="is_sale && order.state == '10'">
				<tui-button type="black" :plain="true" width="148rpx" height="56rpx" :size="26" shape="circle">确认订单</tui-button>
			</view>

			<view class="tui-btn-mr pr" @tap="onOkOrder11" v-if="!is_sale && order.state == '11'">
				<tui-button type="black" :plain="true" width="148rpx" height="56rpx" :size="26" shape="circle">确认订单</tui-button>
			</view>
			

			<view class="tui-btn-mr pr" @tap="onDelOrder" v-if="order.state == '1' || order.state == '10' || order.state == '11' || order.state == '4'|| order.state == '6'">
				<tui-button type="black" :plain="true" width="148rpx" height="56rpx" :size="26" shape="circle">删除订单</tui-button>
			</view>
			
			<view class="tui-btn-mr pr" v-if="order.state == '1'" @tap="onPayOrder">
				<tui-button type="danger" :plain="true" width="148rpx" height="56rpx" :size="26" shape="circle">立即支付</tui-button>
			</view>
			
			<view class="tui-btn-ml pr" v-if="order.state=='3'"  @tap="setOrderState4">
		...
		methods: {
			...
			reLoad() {
				...
				this.app.post('api/order/find_order', {
					...
					//查看了订单
					this.app.post('api/order/order_see', {
						order_no: this.order_no
					}, json => {
					})
			...
			onReOrderProduct: function(e) {

				this.app.msg('确认发起退货', '', is => {
					if(is){
						this.app.post('api/order/set_order_state', {
							order_no: this.order_no,
							state: '52',
