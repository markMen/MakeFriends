...
<!-- 				<tui-list-cell padding="2rpx 30rpx">
		...
		<scroll-view :enable-back-to-top="true" class="tui-scrollList" 
		scroll-y :scroll-into-view="scrollViewId"
			:style="{height:winHeight + 'px'}" 
			:refresher-enabled="false"
			:refresher-triggered="is_triggered"
			@refresherrefresh="onRefreshFriend" 
			...
			<view class="" id="top">
				<tui-list-cell padding="2rpx 30rpx" >
					...
					<view class="tui-name" @tap="onVerify" >好友请求</view>
					<view class="tui-msg-right tui-right-dot mr-lg">
						<view class="tui-msg-time"></view>
			...
			<view class="tui-list city-list">				
				<block v-for="(list,index) in lists" :key="index">
					<view class="tui-list-cell-divider" :id="list.letter_tag" v-if="index > 0">
						{{list.letter}}
					</view>
					<view @click="detail" @touchstart="onDelStart" @touchmove="onDelMove" @touchend="onDelEnd" @touchcancel="onDelCancel"
...
<script>
	...
	export default {
		...
		onShow() {
			...
			// setTimeout(() => {
			// 	this.lists.unshift(this.lists[5])
			// 	this.lists.unshift(this.lists[5])
			// 	this.lists.unshift(this.lists[5])
			// 	this.lists.unshift(this.lists[5])
			// 	this.lists.unshift(this.lists[5])
			...
			// 		this.scrollViewId = this.scrollViewId
			// 	}, 1000)
			// }, 2000)
		...
		methods: {
			...
			touchStart(e) {
				...
				if (item) {
					this.scrollViewId = item.letter_tag;
