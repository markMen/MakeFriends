<template>
 ...
 -->
			...
			<view v-for="(item,index) in chat_list" :key="index" class="m-item " :id="'chat'+item.id">
				<view class="tui-chat-left" v-if="item.from!=me.id && !(item.is_hidden == true)">
					<image :src="item.avatar" class="tui-user-pic tui-right" @tap.stop="onAvatar" :data-id='item.creby'
						:data-user_type='you.user_type'>
					</image>
					<view class="cols full-width">
						<view class="text-color-9 text-xs">{{item.nickname}} {{item.credate}}</view>
				...
				<view class="tui-chat-right" v-if="item.from==me.id  && !(item.is_hidden == true)">
					<view class="cols full-width">
						<view class="text-color-9 text-xs tui-chat-right-time">{{item.credate}} {{item.nickname}}</view>
					...
					<image :src="item.avatar" class="tui-user-pic tui-left" @tap.stop="onAvatar" :data-id='item.creby'
	...
	export default {
		...
		onLoad: function(options) {
			...
			this.app.getUser(user => {
				...
				//板块
				// this.app.post('api/chat_group/chat_user_info', {
				...
				this.app.post('api/chat_group/info', {
					...
					if(code == 0){
						...
						this.app.setTitle(this.chat_group.nickname)
						this.group_user_me = data.group_user_me
					}
				})
				...
				this.app.loopTask(task => {
					if(task.red_group_chat_count > 0){
						this.onChatNext()
					}
				})
		...
		methods: {
			...
			onAvatar(e) {
				...
				if (user_type == 1) {
					...
					this.app.go('/pages/chat_group/group_user_info', {
						user_id: id,
						group_id: this.chat_group.id,
					})
				}
			},
			reLoad: function() { //重新 加载聊天
				//板块
				/*                this.app.post('api/chat_group/chat_user_all', {
				                    user_id: this.user_id,
				                }, json => {
				                    // this.me = json.me
				                    // this.you = json.you
			...
			send(e) { //点击发送
				this.app.post('api/chat_group/chat_send?XDEBUG_SESSION_START=13656', {
					group_id: this.chat_group.id,
					user_id: this.user_id,
					me_id: this.me.id,
			...
			onUploadVoice() { //上传  语音
				this.app.uploadFile('api/chat_group/chat_send', {
					name: 'file',
					path: this.voicePath,
					data: {
						type: '2',
			...
			onPayAudio(e) { //点击了 语音
				...
				if (item.is_click != 1) {
					...
					this.app.post('api/chat_group/chat_click', {
						chat_id: item.id,
					}, json => {
				...
				if (subY > 200 || subY < -200) {
					...
					that.$nextTick(() => {
						setTimeout(() => {
							...
							that.app.uploadFile('api/chat_group/chat_send', {
								name: 'file',
								path: that.voicePath,
								data: {
									type: '2',
			...
			onAddImg(e) { //点击添加图片
				this.app.chooseImage(fs => {
					...
					ufs.forEach(f => {
						...
						this.app.uploadFileTask('api/chat_group/chat_send', file, task => {
							console.log(task)
							chat.per = task.per
						}, () => {
			...
			onAddVideo(e) { //点击添加视频
				this.app.chooseVideo((video, res) => {
					...
					this.app.uploadFileTask('api/chat_group/chat_send?XDEBUG_SESSION_START=17577', videoUp, task => {
						console.log(task)
						chat.per = task.per
					}, () => {
						//this.reLoad()
			...
			onChatPre() { //加载聊天历史
				...
				this.app.post('api/chat_group/chat_user_pre', {
					...param
				}, json => {
			...
			onChatNext(action = null) { //下载下面
				...
				// }
				this.app.post('api/chat_group/chat_user_next', {
					...param
				}, json => {
			...
			onAddLocation(e) { //获取地址
				this.app.chooseLocation(json => {
					this.app.post('api/chat_group/chat_send', {
						type: '6',
						user_id: this.user_id,
						me_id: this.me.id,
			...
			onMenuInvoke(e) { //菜单点击回调
				...
				if (menu_text == '复制') { //点击了复制
					...
					if(this.config.is_admin_back_msg == '1'){
						//是管理员
						if(this.group_user_me.is_admin || this.group_user_me.is_main){
							
							this.app.msgSys('确定撤回消息', is => {
								if (is) {
									this.app.post('api/chat_group/chat_back', {
										group_id:this.group_id,
										id: id,
										XDEBUG_SESSION_START:12636,
									}, json => {
										if (json.code == '0') {
											this.app.toast('撤回成功')
					...
					this.app.msgSys('确定删除消息', is => {
						if (is) {
							this.app.post('api/chat_group/chat_del', {
								id: id,
							}, json => {
								this.app.toast('删除成功')
								selected.is_hidden = '1'
			...
			onAllBackCancel() { //
				...
				this.app.msgSys('确定撤回选中消息', is => {
					if (is) {
						this.app.post('api/chat_group/chat_back_all', {
							ids: ids,
							group_id:this.group_id,
						}, json => {
							if (json.code == '0') {
			...
			onAllDel() { //
				...
				this.app.msgSys('确定删除选中消息', is => {
					if (is) {
						this.app.post('api/chat_group/chat_del_all', {
							ids: ids,
						}, json => {
							this.app.toast('删除成功')
							for (let i in this.chat_list) {
			...
			handlerSuccess(res) {  //录音完成并上传
				...
				setTimeout(()=> {
					that.app.uploadFileH5('api/chat_group/chat_send', {
						name: 'file',
						path: res.data,
						data: {
							type: '2',
		...
		onNavigationBarButtonTap(e) {
			...
			this.app.go('/pages/chat_group/setting', {group_id: this.group_id})
		},
