<template>
	<view class="tui-page">
		...
		<view class="top search-top-up">
			<vive-search type="left" @click="onSearchAll"></vive-search>
		</view>
		<view class="search-top-down"></view>


		<scroll-view :enable-back-to-top="true" class="tui-scrollList" scroll-y :scroll-into-view="scrollViewId"
			:style="{height:winHeight + 'px'}" :refresher-enabled="false" :refresher-triggered="is_triggered"
			@refresherrefresh="onRefreshFriend" @refresherpulling="onPulling" @refresherrestore="onRestore"
			...
			<view class="tui-list city-list">
				<block v-for="(list,index) in lists" :key="index">
					...
					<view @click="detail" @touchstart="onDelStart" @touchmove="onDelMove" @touchend="onDelEnd"
						@touchcancel="onDelCancel" :data-user_id='item.user_id' :data-nickname="item.nickname"
						v-for="(item,index2) in list.data" :key="index2">
						<tui-cell :last="last(list.data,index2)">
							...
							<view class="tui-msg-right" :class="[item.level==3?'tui-right-dot':'']">
								...
								<checkbox-group name="" @change="changeCheck(item)">
									<label @tap.stop="" class="" style="padding: 50rpx 80rpx 50rpx 50rpx;margin: -50rpx;">
										<checkbox :value="1" v-model="item.check" /><text></text>
									</label>
								</checkbox-group>
							</view>
						</tui-cell>
	...
	export default {
		...
		onLoad: function(options) {			
			...
			setTimeout(() => {
				...
				this.is_triggered = false; //触发onRefresh来加载自己的数据，如果不用这种方式，不要在此改变triggered的值
				this.is_triggering = false; //触发onRefresh来加载自己的数据，如果不用这种方式，不要在此改变triggered的值
		...
		methods: {
			...
			onSearch(invoke) {
				...
				if(this.group_id){
					if(this.type == 'del'){
						//板块
						this.app.post('api/chat_contact/list_group_group_id_del?XDEBUG_SESSION_START=15559', {
							...
							for (let i in list) {
								...
								for (let j in item.data) {
									list[i].data[j].check = '0'
								}
							}
							this.lists = list
							this.list_count = list_count
							this.list_count_is = list_count_is
							if (invoke) invoke()
						})
					}else if(this.type == 'add'){
						//板块
						this.app.post('api/chat_contact/list_group_group_id_add?XDEBUG_SESSION_START=19515', {
							search_text: this.search_text,
							group_id: this.group_id,
						}, json => {
							let {
			...
			onDelStart(e) {
				...
				this.touch_type = true
				if (this.action_del_user) {
					clearTimeout(this.action_del_user)
				}


				this.action_del_user = setTimeout(() => {
					if (this.touch_type) {
						this.app.message('确定删除', this.app.ed(e).nickname, is => {
							let user_id = this.app.ed(e).user_id
							//板块
			...
			onDelMove(e) {
				...
				if ((this.time_del_end - this.time_del_start) > 600) {
					this.touch_type = false
		...
		onNavigationBarButtonTap(e) {
			...
			for (let i in this.lists) {
				...
				for (let j in item.data) {
					let obj = item.data[j]
					if (obj && obj.check == '1') {
						user_id_list.push(obj.user_id)
					}
				}
			}
			console.log(user_id_list)
			if(this.type == 'create'){
				this.app.post('api/chat_group/group_add', {
					user_id_list: this.app.toJson(user_id_list),
				}, json => {
				...
				this.app.post('api/chat_group/group_add_user', {
					group_id:this.group_id,
					user_id_list: this.app.toJson(user_id_list),
				}, json => {
					const {code ,data} = json
				...
				this.app.message('系统提示', '确认删除选中的用户', () => {
					this.app.post('api/chat_group/group_del_user', {
						group_id:this.group_id,
						user_id_list: this.app.toJson(user_id_list),
					}, json => {
						const {code ,data} = json
