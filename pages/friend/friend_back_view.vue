<template>
	<view class="tui-page ">
		...
		<view :enable-back-to-top="true" class="tui-scrollList" scroll-y :scroll-into-view="scrollViewId"
			:refresher-enabled="false" :refresher-triggered="is_triggered" @refresherrefresh="onRefreshFriend">

			<!--联系人列表-->
			<view class="tui-list city-list">
				<block v-for="(list,index) in lists" :key="index">
					<view class="tui-list-cell-divider" :id="list.letter" :class="list.letter" v-if="list.letter">
						{{list.letter}}
					</view>
					<view @click="detail" :data-user_id='item.user_id' :data-nickname="item.nickname"
						v-for="(item,index2) in list.data" :key="index2" @longpress="onDelFriend">
			...
			touchStart(e) {
				...
				if (item) {
					...
					this.$nextTick(function(){
						...
						// let view = query.select('#' + item.letter).boundingClientRect(data => {
						// 	console.log(data)
						// 	uni.pageScrollTo({
						// 		duration: 300,
						// 		scrollTop:data.top + 134,
						// 	})
						// })
						// let view = query.select('#' + item.letter)
						
						query.select('#' + item.letter).fields({
							size: true,
							rect: true,
							scrollOffset:true
						}, (data) => {
