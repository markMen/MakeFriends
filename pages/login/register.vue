                ...
                <view class="rows pt-lg">
                    ...
                    <view class="rows text-sm">
                        <checkbox-group @change="checkboChangeInterface">
                            <label class="checkbox checkbox-warning checkbox-main">
...
/*                this.app.uploadFilesHtml('api/login/upload', this.company_img, uploadFiles => { //上传完成后
				...
				if(this.model.id){
					...
					this.app.postHtml('api/login/register_phone_shot', {
					    ...this.model,
					}, json => {
						let {code, data} = json						
					    if (code == '0') {
							
							this.app.postHtml('api/user/me_info', {openid: data.openid}, json => {
								let {code, data} = json
								if(code == '0'){
									uni.setStorageSync("thorui_mobile", (data.phone));
									uni.setStorageSync("phone", (data.phone));
									uni.setStorageSync('did', data.did)
									
									this.app.setUser(data)
									this.app.toast("登录成功");
									this.app.go('/pages/login/register_finish')
								}else{
									this.app.toast(data);
								}
							})
							...
							// }
							// actionCheck()
					        // this.app.goc('/pages/login/register_finish', {openid: data.openid})
					    } else {
					        this.app.msgSys(data)
					    }
					})
				}
